...
fn migrate_to_1(conn: &Connection) -> Result<()> {
    ...
    if schema_mentions(conn, "node", "token_hash")? {
        ...
        info!("prepared legacy token column for migration");
    }
...
fn migrate(conn: &Connection, from: i64) -> Result<()> {
    ...
    let conn = &*tx;
    // Schema, not token shape, distinguishes legacy hashes from raw tokens.
    let legacy_hashes = from == 0 && columns_of(conn, "node")?.contains("token_hash");
    if from < 1 {
        migrate_to_1(conn)?;
    }
    if from < 2 {
    ...
    if from < 5 {
        ...
        conn.execute("ALTER TABLE node RENAME COLUMN token TO token_hash", [])?;
        if !legacy_hashes {
            let rows = {
                let mut stmt = conn.prepare("SELECT id, token_hash FROM node")?;
                let rows = stmt
            ...
            for (id, token) in rows {
                conn.execute("UPDATE node SET token_hash=?2 WHERE id=?1", params![id, sha256(&token)])?;
            }
        }
    }
...
///
/// It is the credential store: node credential hashes, the GitHub client
...
impl Db {
    pub fn open(path: &str) -> Result<Self> {
        ...
        anyhow::ensure!(version <= SCHEMA_VERSION, "database schema is newer than this server");
        conn.execute_batch(SCHEMA)?;
        restrict(path);
        migrate(&conn, if fresh { SCHEMA_VERSION } else { version })?;
        // Remove historical WAL frames after migration. Existing backups remain sensitive.
        let busy: i64 = conn.query_row("PRAGMA wal_checkpoint(TRUNCATE)", [], |r| r.get(0))?;
        anyhow::ensure!(busy == 0, "database is busy; stop other connections before starting romi");
    ...
    pub fn reset_token(&self, id: i64, token: &str) -> Result<()> {
        self.conn().execute("UPDATE node SET token_hash=?2 WHERE id=?1", params![id, sha256(token)])?;
        Ok(())
    }
    ...
    pub fn node_by_token(&self, token: &str) -> Result<Option<i64>> {
        Ok(self
            .conn()
            .query_row("SELECT id FROM node WHERE token_hash = ?1", [sha256(token)], |r| r.get(0))
            .optional()?)
    }
    ...
    pub fn backup_into(&self, dest: &str) -> Result<()> {
        ...
        // The copy is the credential store in one portable file: credential hashes, the GitHub secret, the password hash. SQLite creates it
...
fn row_to_node(r: &rusqlite::Row<'_>) -> Node {
    ...
    Node {
        id: n("id"),
        name: s("name"),
        public: r.get::<_, bool>("public").unwrap_or(false),
        sort: n("sort"),
...
mod tests {
    ...
    #[test]
    fn token_migration_preserves_authentication_and_rolls_back_on_failure() {
        for legacy in [false, true] {
            let old = Connection::open_in_memory().unwrap();
            old.execute_batch(SCHEMA).unwrap();
            let raw = "existing-agent-secret";
            if !legacy {
                old.execute_batch("ALTER TABLE node RENAME COLUMN token_hash TO token").unwrap();
            }
            let column = if legacy { "token_hash" } else { "token" };
            old.execute(
                &format!("INSERT INTO node(name,{column},created_at) VALUES ('old',?1,0)"),
                [if legacy { sha256(raw) } else { raw.into() }],
            )
            .unwrap();
            migrate(&old, if legacy { 0 } else { 4 }).unwrap();
            let id: i64 = old
                .query_row("SELECT id FROM node WHERE token_hash=?1", [sha256(raw)], |r| r.get(0))
                .unwrap();
            assert!(id > 0);
            migrate(&old, SCHEMA_VERSION).unwrap();
            assert_eq!(
                old.query_row("SELECT token_hash FROM node", [], |r| r.get::<_, String>(0)).unwrap(),
                sha256(raw)
            );
        }
        let broken = Connection::open_in_memory().unwrap();
        broken.execute_batch(SCHEMA).unwrap();
        broken.execute_batch("ALTER TABLE node RENAME COLUMN token_hash TO token; PRAGMA user_version=4; CREATE TRIGGER refuse_hash BEFORE UPDATE ON node BEGIN SELECT RAISE(ABORT, 'test failure'); END;").unwrap();
        broken.execute("INSERT INTO node(name,token,created_at) VALUES ('old','secret',0)", []).unwrap();
        assert!(migrate(&broken, 4).is_err());
        assert!(columns_of(&broken, "node").unwrap().contains("token"));
        assert_eq!(broken.query_row("PRAGMA user_version", [], |r| r.get::<_, i64>(0)).unwrap(), 4);
        broken.execute_batch("DROP TRIGGER refuse_hash").unwrap();
        migrate(&broken, 4).unwrap();
    }

    #[test]
    fn restoring_a_v4_backup_hashes_tokens_before_copying() {
        let path = std::env::temp_dir().join(format!("romi-v4-{}.db", crate::auth::random_token()));
        let old = Connection::open(&path).unwrap();
        old.execute_batch(SCHEMA).unwrap();
        old.execute_batch("ALTER TABLE node RENAME COLUMN token_hash TO token; PRAGMA user_version=4;")
            .unwrap();
        old.execute("INSERT INTO node(name,token,created_at) VALUES ('old','legacy-secret',0)", []).unwrap();
        drop(old);
        let db = db();
        db.check_backup(path.to_str().unwrap()).unwrap();
        db.restore_from(path.to_str().unwrap()).unwrap();
        assert!(db.node_by_token("legacy-secret").unwrap().is_some());
        assert!(db.node_by_token(&sha256("legacy-secret")).unwrap().is_none());
    ...
    fn a_backup_restores_the_database_it_was_taken_from() {
        ...
        assert_eq!(db.node_by_token("token-kept").unwrap(), Some(back[0].id));
    ...
    fn tokens_are_hashed_and_rotation_retires_the_old_one() {
        ...
        let stored: String =
            db.conn().query_row("SELECT token_hash FROM node WHERE id=?1", [id], |r| r.get(0)).unwrap();
        assert_eq!(stored, sha256("first-token"));
        assert_eq!(db.node_by_token(&stored).unwrap(), None);
    ...
    fn dropping_load1_keeps_the_history_and_lets_new_rows_in() {
        ...
        let old = Connection::open(path).unwrap();
        old.execute_batch(SCHEMA).unwrap();
        old.execute_batch("DROP TABLE metric; ALTER TABLE node RENAME COLUMN token_hash TO token;").unwrap();
        old.execute_batch(
            "CREATE TABLE metric (

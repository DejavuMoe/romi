...
fn locate(app: Shared, node_id: i64, ip: String) {
    if app.db.get("country_lookup").as_deref() != Some("on") {
        return;
    }
    ...
    tokio::spawn(async move {
        let lookup = async {
            anyhow::ensure!(app.db.get("country_lookup").as_deref() == Some("on"), "country lookup disabled");
...
mod tests {
    ...
    fn country_lookup_is_not_scheduled_without_opt_in() {
        ...
        locate(app, i64::MAX, "127.0.0.1".into());
        assert!(ASKED.get().is_none_or(|asked| !asked.lock().unwrap().contains_key(&i64::MAX)));
    }

...
/// Version 3 adds hourly history; sessions remain excluded.
const BACKUP_FORMAT: i64 = 3;
...
fn build_staging(inner: &Arc<Inner>, src: &str, dest: &str, floor: &[(String, i64)]) -> Result<BackupReport> {
    ...
    let outcome = (|| -> Result<BackupReport> {
        ...
        let indexed_rows = report
            ...
            .saturating_add(report.rows.get("metric_hour").copied().unwrap_or(0))
            .saturating_add(report.rows.get("ping_hour").copied().unwrap_or(0));
        ...
        for table in BACKUP_TABLES {
            ...
            let mut ours = native_columns(&conn, table)?;
            if report.schema < 3 {
                let added: &[&str] = match table {
                    "node" => &[
                        "traffic_unit",
                        "priority",
                        "bandwidth_up",
                    ...
                    "metric" => &["zram_used", "swap_disk_used", "swapfile_used", "swap_partition_used"],
                    "metric_hour" => &[
                        "tcp_sum",
                        "tcp_samples",
                        "udp_sum",
                        ...
                        "zram_used_samples",
                        "swap_disk_used_sum",
                        "swap_disk_used_samples",
                        "swapfile_used_sum",
                        "swapfile_used_samples",
                        "swap_partition_used_sum",
                        "swap_partition_used_samples",
                    ],
                    _ => &[],
                };
                ours.retain(|(name, _)| {
                    !added.contains(&name.as_str()) || theirs.iter().any(|(n, _)| n == name)
                });
            }
...
pub(super) fn verify_relationships(conn: &Connection) -> Result<()> {
    let checks = [
        (
            "traffic 指向不存在的节点",
            "SELECT COUNT(*) FROM traffic t LEFT JOIN node n ON n.id=t.node_id WHERE n.id IS NULL",
        ),
        (
            "metric 指向不存在的节点",
            "SELECT COUNT(*) FROM metric m LEFT JOIN node n ON n.id=m.node_id WHERE n.id IS NULL",
        ),
        (
            "metric_hour 指向不存在的节点",
            "SELECT COUNT(*) FROM metric_hour m LEFT JOIN node n ON n.id=m.node_id WHERE n.id IS NULL",
        ),
        (
            "ping_hour 指向不存在的节点",
            "SELECT COUNT(*) FROM ping_hour r LEFT JOIN node n ON n.id=r.node_id WHERE n.id IS NULL",
        ),
        (
            "ping_hour 指向不存在的探测",
            "SELECT COUNT(*) FROM ping_hour r LEFT JOIN ping_task p ON p.id=r.task_id WHERE p.id IS NULL",
        ),
        (
            "ping_node 指向不存在的节点",
            "SELECT COUNT(*) FROM ping_node p LEFT JOIN node n ON n.id=p.node_id WHERE n.id IS NULL",
        ),
        (
            "ping_node 指向不存在的探测",
            "SELECT COUNT(*) FROM ping_node p LEFT JOIN ping_task t ON t.id=p.task_id WHERE t.id IS NULL",
        ),
        (
            "ping_record 指向不存在的节点",
            "SELECT COUNT(*) FROM ping_record r LEFT JOIN node n ON n.id=r.node_id WHERE n.id IS NULL",
        ),
        (
            "ping_record 指向不存在的探测",
            "SELECT COUNT(*) FROM ping_record r LEFT JOIN ping_task p ON p.id=r.task_id WHERE p.id IS NULL",
        ),
        (
            "有节点没有 traffic 行",
            "SELECT COUNT(*) FROM node n LEFT JOIN traffic t ON t.node_id=n.id WHERE t.node_id IS NULL",
        ),
        (
            "node.token_hash 重复",
            "SELECT COUNT(*) FROM (SELECT token_hash FROM node GROUP BY token_hash HAVING COUNT(*) > 1)",
        ),
    ];

...
/// closing the same gap `auth::PASSWORD_GATE` closes on
/// the anonymous login path. Every request here holds the single connection the agents
...
pub async fn me(State(app): State<Shared>, headers: HeaderMap) -> Json<Value> {
    Json(json!({
        ...
        "custom_themes_allowed": app.allow_custom_themes,
...
pub async fn upload_theme(
    ...
    if !app.allow_custom_themes {
        return (StatusCode::FORBIDDEN, "custom themes are disabled").into_response();
    }
...
async fn update(app: &App, short: &str) -> Result<(bool, String), anyhow::Error> {
    ...
    anyhow::ensure!(app.allow_custom_themes, "custom themes are disabled");
...
pub async fn theme_preview(_: Admin, State(app): State<Shared>, Path(short): Path<String>) -> Response {
    match crate::frontend::preview(&app.themes, &short, app.allow_custom_themes) {
...
pub async fn themes(_: Admin, State(app): State<Shared>) -> Response {
    match crate::frontend::themes(&app) {
        Ok(themes) => {
            Json(json!({"themes": themes, "custom_themes_allowed": app.allow_custom_themes})).into_response()
        }
        Err(e) => fail(e),
    }
}
...
mod tests {
    ...
    async fn private_defaults_and_theme_denials_are_enforced_at_the_api() {
        ...
        assert_eq!(
            update_theme(Admin, State(app.clone()), Path("default".into())).await.status(),
            StatusCode::FORBIDDEN
        );
        assert_eq!(
            delete_theme(Admin, State(app.clone()), Path("default".into())).await.status(),
            StatusCode::FORBIDDEN
        );
        assert_eq!(
            upload_theme(
        ...
        for key in ["public_page", "country_lookup"] {
    ...
    /// `PASSWORD_GATE` enforces on the anonymous login path

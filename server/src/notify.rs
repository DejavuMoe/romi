...
pub async fn test(_: Admin, State(app): State<Shared>, Query(target): Query<TestTarget>) -> Response {
    if target.channel.as_deref().is_some_and(|v| !matches!(v, "telegram" | "webhook")) {
        return (StatusCode::BAD_REQUEST, "invalid channel").into_response();
    }
    let channels: Vec<_> = channels(&app)
        .into_iter()
        .filter(|c| target.channel.as_deref().is_none_or(|name| name == c.name()))
        .collect();

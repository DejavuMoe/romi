...
pub struct App {
    ...
    /// Expensive password verification is bounded per Hub instance. Keeping the
    /// permit in application state isolates independent test/runtime instances
    /// without changing the production limit.
...
impl App {
    fn new(db: Db, site: String, data_dir: PathBuf, notes: tokio::sync::mpsc::Sender<notify::Note>) -> Self {
        Self {
            geo: geo::Geo::new(data_dir.join("GeoLite2-Country.mmdb")),
            db,
    ...
    while let Some(arg) = it.next() {
        ...
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                println!(
                    "{}\n\n\
                     Usage: romi-hub [--listen 127.0.0.1:28080] [--db romi.db] [--site https://hub.example.com]\n\n\
                     --version prints the romi version and exits without opening a database.\n\
                     --listen defaults to 127.0.0.1:28080.\n\
                     Only the built-in default theme is supported.\n\
...
async fn main() -> Result<()> {
    ...
    let mut app = App::new(
        Db::open_with(&args.database, options)?,
        args.site.clone(),
        StdPath::new(&args.database).parent().unwrap_or_else(|| StdPath::new(".")).to_path_buf(),
        notes,
    );
    ...
    let router = Router::new()
        ...
        .route("/api/geolite", get(geo::status).post(geo::update).delete(geo::cancel))
...
fn automatic_maintenance(app: &App, now: i64) -> Result<()> {
    let days = app
        .db
        .get("maintenance_days")
        .and_then(|v| v.parse::<i64>().ok())
        .filter(|d| [7, 30, 90, 180].contains(d))
        .unwrap_or(0);
    if days == 0 {
        return Ok(());
    }
    let Some(last) = app.db.get("maintenance_last").and_then(|v| v.parse::<i64>().ok()) else {
        return app.db.set("maintenance_last", &now.to_string());
    };
    if now.saturating_sub(last) >= days * 86400 {
        app.db.maintenance(app.db.retention_days())?;
        app.db.set("maintenance_last", &now.to_string())?;
    }
    Ok(())
}
...
async fn housekeeping(app: Shared) {
    ...
    loop {
        ...
        if let Err(e) = tokio::task::spawn_blocking(move || {
            ...
            if let Err(e) = automatic_maintenance(&app, chrono::Utc::now().timestamp()) {
                warn!("automatic maintenance failed: {e:#}");
            }
        ...
        .await
        {
            warn!("housekeeping task failed: {e}");
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn automatic_maintenance_is_off_by_default_and_runs_only_when_due() {
        let app = app("");
        automatic_maintenance(&app, 1000).unwrap();
        assert!(app.db.get("maintenance_last").is_none());
        app.db.set("maintenance_days", "7").unwrap();
        automatic_maintenance(&app, 1000).unwrap();
        automatic_maintenance(&app, 1000 + 7 * 86400 - 1).unwrap();
        assert_eq!(app.db.get("maintenance_last").as_deref(), Some("1000"));
        automatic_maintenance(&app, 1000 + 7 * 86400).unwrap();
        assert_eq!(app.db.get("maintenance_last").unwrap(), (1000 + 7 * 86400).to_string());
        automatic_maintenance(&app, 999).unwrap();
        assert_eq!(app.db.get("maintenance_last").unwrap(), (1000 + 7 * 86400).to_string());
    }

...
pub async fn serve(State(app): State<Shared>, headers: HeaderMap, uri: Uri) -> Response {
    ...
    if path == "admin" || path.starts_with("admin/") {
        ...
        return embedded::<AdminAssets>(path, "the panel is not built; run `npm run build` in admin/", known);
    }

    let theme = app.db.get("theme").unwrap_or_default();
    if let Some(root) = app.allow_custom_themes.then(|| external_theme(&app.themes, &theme)).flatten() {
        if let Some(response) = disk(&root, path, known) {
            return response;
        }
    }
    embedded::<DefaultThemeAssets>(
        path,
        "the default theme is missing; run `make frontend` at the repository root",
        known,
    )
}
...
pub fn themes(app: &App) -> std::io::Result<Vec<Theme>> {
    ...
    if let Some(base) = app.allow_custom_themes.then(|| app.themes.canonicalize().ok()).flatten() {
...
pub fn preview(themes: &Path, short: &str, allow_custom: bool) -> Option<Vec<u8>> {
    if !valid_short(short) {
        return None;
    }
    if !allow_custom {
        return (short == "default")
            .then(|| DefaultPreview::get(PREVIEW).map(|file| file.data.into_owned()))
            .flatten();
    }
...
mod tests {
    ...
    fn an_uploaded_theme_is_checked_before_it_replaces_the_one_in_place() {
        ...
        .unwrap();
        assert_eq!(preview(&base, "aurora", true).unwrap(), b"PNG");
        install(&base, pack(&[("theme.json", manifest), ("dist/index.html", b"v3")], None), None).unwrap();
        assert!(preview(&base, "aurora", true).is_none());
        ...
        assert_ne!(preview(&base, "default", false), Some(b"DISK".to_vec()));
        app.allow_custom_themes = true;

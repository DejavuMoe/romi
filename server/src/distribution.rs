...
impl Distribution {
    ...
    pub fn load(directory: &Path, expected_version: &str) -> Result<Self> {
        ...
        if size == 0 {
            bail!("{}: size must be positive", metadata_path.display());
        }
        ...
        if !is_x86_64_elf(&bytes) {
            bail!("{}: binary is not a 64-bit x86-64 ELF", binary_path.display());
        }

        let download_path = format!("/agent/v{}/{}", version, architecture);
        Ok(Self { version, target, architecture, sha256, size, download_path, binary: Bytes::from(bytes) })
    }
...
impl Distribution {
    ...
    pub(crate) fn test_fixture(version: &str, bytes: &[u8]) -> Self {
        Self {
            ...
            download_path: format!("/agent/v{version}/{ARCHITECTURE}"),
...
mod tests {
    ...
    fn a_matching_local_distribution_loads_and_exposes_no_path() {
        ...
        assert_eq!(distribution.download_path, "/agent/v0.1.0/x86_64");

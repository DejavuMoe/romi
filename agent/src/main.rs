    ...
    eprintln!(
        "{}\n\n\
         ...
         Options:\n  \
           ...
           --interval <secs>    Report interval (default 3, or ROMI_INTERVAL)\n  \
           --iface <list>       Traffic interfaces, comma separated; full names\n  \
                                select only those interfaces, while -name removes\n  \
                                one from the default set (or ROMI_IFACE).\n  \
...
fn parse_args() -> Result<Args> {
    let (mut server, mut token, mut interval, mut iface, mut insecure) = (None, None, None, None, false);
    ...
    // its ExecStart argv.
    let interval = report_interval(interval, std::env::var("ROMI_INTERVAL"))?;
    let iface = iface.or_else(|| std::env::var("ROMI_IFACE").ok()).unwrap_or_default();
    let ifaces = Ifaces::parse(&iface).map_err(anyhow::Error::msg)?;
    Ok(Args { server, token, interval, ifaces, insecure })
}

fn report_interval(cli: Option<u64>, env: std::result::Result<String, std::env::VarError>) -> Result<u64> {
    let value = match cli {
        Some(value) => value,
        None => match env {
            Ok(value) => value.parse::<u64>().context("ROMI_INTERVAL must be an integer")?,
            Err(std::env::VarError::NotPresent) => 3,
            Err(error) => return Err(error.into()),
        },
    };
    anyhow::ensure!((3..=60).contains(&value), "report interval must be 3-60 seconds");
    Ok(value)
}
...
mod interval_tests {
    use super::*;
    #[test]
    fn interval_defaults_bounds_and_override_are_explicit() {
        assert_eq!(report_interval(None, Err(std::env::VarError::NotPresent)).unwrap(), 3);
        assert_eq!(report_interval(Some(5), Ok("invalid".into())).unwrap(), 5);
        for value in ["-1", "0", "1", "2", "61", "1.5", "1e1", "invalid"] {
            assert!(report_interval(None, Ok(value.into())).is_err());
        }
        for value in ["3", "60"] {
            assert!(report_interval(None, Ok(value.into())).is_ok());
        }
    }
}

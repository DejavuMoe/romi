...
// Shell snippets must preserve quotes and shell metacharacters as literal arguments.
const { shellQuote, agentCommand, registrationCommand } = await import("./api.ts")
...
assert.equal(agentCommand("http://remote.test", 1), "")
assert.equal(agentCommand("https://romi.test", 0), "tmp=$(mktemp) && trap 'rm -f \"$tmp\"' EXIT && curl -fsSL 'https://romi.test/install.sh' -o \"$tmp\" && sudo sh \"$tmp\" --server 'https://romi.test' --interval 1")
assert.ok(!agentCommand("https://romi.test", 1).includes("secret"))
assert.ok(!agentCommand("https://romi.test", 1).includes("ROMI_TOKEN"))
assert.ok(registrationCommand("https://romi.test", "key").includes("--register-key 'key'"))
assert.ok(registrationCommand("https://romi.test", "key").includes("mktemp"))

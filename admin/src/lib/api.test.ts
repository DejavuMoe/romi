...
for (const seconds of [0,1,2,3.5,61,NaN]) assert.equal(agentCommand("https://romi.test",seconds),"")
assert.ok(agentCommand("https://romi.test",3).endsWith("--interval 3"))
assert.ok(!agentCommand("https://romi.test", 3).includes("secret"))
assert.ok(!agentCommand("https://romi.test", 3).includes("ROMI_TOKEN"))

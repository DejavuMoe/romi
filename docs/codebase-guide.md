...
`/admin/` 规范化为 `/admin/nodes`；后台各区有独立路径，可刷新或直接打开。未登录时显示账号密码表单及已配置时的 GitHub 登录入口。桌面侧栏与移动端弹窗导航指向节点、监测、通知、数据、安全、设置；顶部提供公开页、明暗切换、退出登录。表单有忙态和错误提示，删除、令牌换发、维护、恢复使用确认弹窗；节点弹窗关闭后把焦点还给触发控件或主内容。
...
| 安全 `/admin/security` | 查看/撤销其他会话、配置 GitHub OAuth 与允许用户名、修改本地账号和密码 | `Security`、`Sessions` |
| 设置 `/admin/settings` | 站点名称、分钟保留天数、连续在线重置阈值、公开页开关与默认视图、GeoLite Country 下载/取消 | `SettingsTab`、`GeoSettings` |

节点编辑只提交改动过的字段。额度按 GB/TB 输入并转换为字节，`0` 表示不限；流量修正区的未改字段不会覆盖累计量。价格、币种、付款周期与到期日期在独立账单表单中编辑。优先级为 `0–999999`，数字越大越靠前；当前后台以数值输入调整它。API 仍有整表 `PUT /api/nodes/order`，可一次写入排序及优先级。节点级 `public` 字段可由 API 更新，**当前后台节点表单没有单独的公开/私有开关**；后台只显示私有标记。以上按当前组件代码记录，不能把 API 能力当作已有界面入口。
...
两套前端首先请求 `/api/me` 判定身份和站点配置，再经 `/api/nodes` 取初始节点列表，随后连接 `/api/ws`。Hub 每 2 秒推送一份按受众生成的节点快照；WebSocket 失败时页面每 5 秒轮询，并尝试重新连接。会话被撤销或公开页被关闭后，服务端停止原连接，客户端重新读取身份或转登录页。详情历史按当前标签调用 `/api/nodes/{id}/metrics?hours=&points=&series=`；切换窗口会取消旧请求，避免旧数据覆盖新窗口。共享 `shared/http.ts` 保留 HTTP 状态码，且对 `204 No Content` 不尝试解析 JSON。
...
2. 本地登录同时核对账号与密码。GitHub OAuth 校验回调 `state`，且允许用户名列表为空时拒绝所有人。登录发放 14 天会话：Cookie 为 HttpOnly、SameSite=Lax，HTTPS 条件满足时标记 Secure；`session` 表只存令牌 SHA-256 摘要。注销、撤销、账号/密码修改会使对应会话失效。

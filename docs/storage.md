...
Schema 2 添加两个小时表。备份格式 3 包含九张持久表（仍不包含 session），另加 manifest；
旧测试版备份格式不在首次公开测试版的兼容范围内。后台维护在阻塞线程执行，不占用 Tokio
异步工作线程。默认 DuckDB 引擎预算仍不是整个 Hub 的 RSS 上限。

## Schema 3

事务式迁移增加优先级、双向可用带宽、IP 协议可用性、额度输入单位与连续在线起点，不改旧节点的 public 值。
分钟历史增加可空的 ZRAM/普通 Swap/Swapfile/交换分区字段；小时历史按各字段的有效样本数加权，也补齐进程数、TCP/UDP 连接数。
旧 Agent/旧小时历史的缺字段保留 null；读取端使用返回的桶宽断开缺失区间，不补零或跨空值连线。
格式 3 的 schema 2 备份可恢复：只允许已知新增列缺失，其余列类型、归档边界、摘要和关系校验保持不变。已有 schema 3 数据不能降级交给旧 Hub；回退应使用升级前备份。
...
- 路径存在但不是 DuckDB 文件：只读取文件头 12 字节即拒绝启动，不加载、改写、重命名或删除原文件；空文件视为新库。
...
如果平台能提供空闲空间信息，构建 staging 数据库前还会做一次咨询性磁盘空间检查（要求至少为展开大小的两倍再加 16 MiB）；
它不构成预留，最终仍以真实写入错误为准。

## 恢复顺序与回滚

恢复按以下顺序执行，把可能失败的工作放在旧库被永久丢弃之前。staging 数据库先以
不带 `metric`、`metric_hour`、`ping_record`、`ping_hour` primary key 的形式建表并批量装入，再通过
...
3. 在同一 staging 数据库中执行恢复变换：构建延迟的主键（重复行即失败）、清除 session 表、删除已退役设置、校验关系、重置并抬高 id 分配器。
...
1. 先把过期的分钟明细汇总进小时表，再删除过期分钟明细与超过 365 天的小时数据（不触碰累计流量）。

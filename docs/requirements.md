...
| ID | 产品契约 | 主要依据 |
| --- | --- | --- |
| R-01 | CPU/负载、RAM/ZRAM/Swap、磁盘、网络、进程、连接数和运行时长；上报间隔 3–60 秒整数，默认 3 秒 | `agent/src/collect.rs`、`agent/src/main.rs` |
...
| R-05 | Telegram/Webhook；离线/恢复、流量、到期与自动续期、后台登录提醒 | `server/src/notify.rs`、管理通知页 |
| R-06 | 管理列表（搜索、在线状态筛选）、地址复制、优先级、账单、可用带宽、协议可用性、安装、批量注册窗口及令牌轮换 | `admin/src/components/sections/nodes.tsx`、`admin/src/components/sections/node-forms.tsx` |

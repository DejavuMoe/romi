...
- 面板统一 16px 内边距；次要按钮（含弹窗的取消/关闭）为透明描边，主按钮为实心；输入框与页面同底色，悬停时描边加深；表头与字段标签为 12px 常规字重；复选项整行可点，复选框在前，手机上整行不低于 44px；
- 弹窗宽 560px，顶部 48px 标题栏带分隔线、滚动时连同关闭按钮固定，正文 16px；767px 以下为贴底的整宽面板；
- 空状态与加载失败是同一种居中描边框，分别以 `[ — ]` 与 `[ ! ]` 标记，后者带重试。

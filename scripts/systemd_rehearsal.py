...
class Rehearsal:
    ...
    def load_and_verify_release(self) -> None:
        ...
        if self.manifest.get("target") != "x86_64-unknown-linux-gnu":
    ...
    def verify_hub_filesystem(self) -> None:
        ...
        expect_mode(state / "themes", 0o750, self.hub_uid, self.hub_gid)
    ...
    def bootstrap_lifecycle(self) -> None:
        ...
        login = self.direct_admin.request("POST", "/api/auth/login", json_body={"password": secret})
        if login.status != 200:
            fail(f"bootstrap credential login returned {login.status}")
        if self.direct_admin.request("GET", "/api/db").status != 200:
            fail("authenticated direct admin request failed after bootstrap login")

        self.new_password = secrets.token_hex(24)
        changed = self.direct_admin.request(
        ...
        stale = HTTPSession(HUB_BASE)
        old_login = stale.request("POST", "/api/auth/login", json_body={"password": secret})
        if old_login.status != 401:
            fail(f"old bootstrap password still authenticates (HTTP {old_login.status})")
        fresh = HTTPSession(HUB_BASE)
        new_login = fresh.request("POST", "/api/auth/login", json_body={"password": self.new_password})
    ...
    def direct_provisioning_is_refused(self) -> None:
        response = self.direct_admin.request(
...
""".lstrip(),
    ...
    def tls_paths(self) -> None:
        ...
        login = client.request("POST", "/api/auth/login", json_body={"password": self.new_password})
        if login.status != 200:
        ...
        expected_download = f"/agent/v{self.version}/x86_64"
        for key, value in (
    ...
    def wait_for_telemetry(self, node_id: int, timeout: float = 45.0) -> dict:
        def predicate():
            ...
            if not isinstance(metrics, dict) or not metrics.get("mem_total"):
                return None
            return node
        ...
        try:
            def predicate():
                for node in self.tls_admin.request("GET", "/api/nodes").json().get("nodes", []):
                    if node["id"] not in before and node.get("online") and node.get("metrics", {}).get("mem_total"):
                        return node
                return None
    ...
    def hub_restart_cycle(self) -> None:
        before_metrics = self.node_metrics(self.permanent_node_id)
        before_points = {row.get("ts") for row in before_metrics.get("metrics", []) if row.get("ts") is not None}
        if not before_points:
            fail("no previously committed telemetry was readable before restart")

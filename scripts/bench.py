        ...
        try:
            while self.alive:
                ...
                if length > 8 * 1024 * 1024:
                    raise OSError('benchmark frame limit exceeded')
...
class Bench:
    ...
    def _run(self):
        args = self.args
        self.hub.start(memory=args.memory, threads=args.db_threads)
        password = self.hub.password()
        if password:
            self.request('/api/auth/login', {'username': 'admin', 'password': password})
        tokens = self.create_nodes()
        recent = {node: deque(maxlen=32) for node in range(1, args.nodes + 1)}
        freshness = []
        unmatched = [0]
        def observe(payload):
            now = time.monotonic()
            for node in json.loads(payload).get('nodes', []):
                history = recent.get(node['id'])
                if history and node.get('metrics'):
                    marker = node['metrics'].get('uptime')
                    for counter, stamp in tuple(history):
                        if counter == marker:
                            freshness.append((now - stamp) * 1000)
                            break
                    else:
                        unmatched[0] += 1
        cookie = '; '.join(f'{c.name}={c.value}' for c in self.cookies)
        viewers = [Socket('127.0.0.1', self.hub.port, '', '/api/ws', cookie, observe if i == 0 else None)
        ...
        def report(node, agent):
            ...
            if args.stagger:
                stop.wait(((node * 0.61803398875) % 1) * args.interval)
            while not stop.is_set():
                ...
                try:
                    recent[node].append((counters, time.monotonic()))
                    agent.text(message)
                except OSError:
                    return
                sent[node] += 1
        ...
        def read_history(node):
            while not stop.is_set():
                start = time.perf_counter()
                try:
                    with self.request(f'/api/nodes/{node}/metrics?hours=24') as response:
                        response.read()
        ...
        committed_before = self.db_stats().get('queue', {}).get('committed_ops_total')
        freshness.clear()
        unmatched[0] = 0
        viewer_bytes_before = sum(v.bytes_received for v in viewers)
        viewer_frames_before = sum(v.text_frames for v in viewers)
        rss_before, cpu_before = self.hub.rss_kb(), self.hub.cpu_seconds()
        started = time.monotonic()
        time.sleep(args.seconds)
        elapsed = time.monotonic() - started
        cpu_after = self.hub.cpu_seconds()
        viewer_bytes = sum(v.bytes_received for v in viewers) - viewer_bytes_before
        viewer_frames = sum(v.text_frames for v in viewers) - viewer_frames_before
        observed = sorted(freshness)
        unmatched_samples = unmatched[0]
        viewer_errors = [v.reader_error for v in viewers if v.reader_error]
        stop.set()
        ...
        committed_after = self.db_stats().get('queue', {}).get('committed_ops_total')
        ...
        result = {
            ...
            'workload': {'nodes': args.nodes, 'interval_s': args.interval, 'readers': args.readers,
                         'seconds': args.seconds, 'target_report_rate': args.rate, 'viewers': args.viewers,
                         'staggered_reports': args.stagger},
            'live_view': {
                'frames': viewer_frames, 'bytes_per_second': round(viewer_bytes / elapsed, 1),
                'report_age_ms': {f'p{p}': round(observed[min(len(observed)-1, int(len(observed)*p/100))], 2)
                                  for p in (50,95,99)} if observed else None,
                'age_samples': len(observed),
                'unmatched_age_samples': unmatched_samples,
                'reader_errors': viewer_errors,
            },
            ...
            'ingest': {
                ...
                'cached_view_report_equivalents': committed_window,
                'cached_view_note': 'node-list totals may lag by the snapshot TTL; not a commit-rate measurement',
                'committed_operations': committed_after - committed_before if committed_before is not None and committed_after is not None else None,
                'operations_note': 'fresh writer counters include telemetry and history writes',
...
def main():
    ...
    parser.add_argument('--viewers', type=int, default=0)
    parser.add_argument('--stagger', action='store_true', help='deterministic distributed Agent phases')
    ...
    parser.add_argument('--db-threads', type=int, default=None, help='DuckDB worker count for controlled ablations')

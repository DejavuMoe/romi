...
def run_self_checks() -> int:
    ...
    for status, stdout, stderr, expected in (
        (2, "", "", False),
        (0, "a" * 40 + "\trefs/tags/v0.1.0\n", "", True),
    ):
        result = subprocess.CompletedProcess(["git"], status, stdout, stderr)
        with mock.patch(__name__ + ".git", return_value=result):
            assert remote_tag_exists("origin", "v0.1.0", ROOT) is expected
    result = subprocess.CompletedProcess(["git"], 128, "", "network failure\n")
    with mock.patch(__name__ + ".git", return_value=result):
        try:

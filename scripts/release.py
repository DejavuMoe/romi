...
from platform_build import IMAGES  # noqa: E402

ROOT = Path(__file__).resolve().parents[1]
TARGETS = tuple(IMAGES)
TARGET = os.environ.get("ROMI_RELEASE_TARGET", "x86_64-unknown-linux-gnu")
FORMAT = 1
PROJECT = "romi"
...
def manifest_filename(kind: str, version: str) -> str:
    if kind == CANDIDATE_KIND:
        return f"romi-release-candidate-v{version}-{TARGET}.json"
    if kind == PUBLIC_KIND:
        return f"romi-release-v{version}-{TARGET}.json"
    ...
    if component == "hub":
        return (
            ...
            *common,
            ...
            ("deploy/hub/romi-hub.openrc.in", "deploy/hub/romi-hub.openrc.in", 0o644),
        )
    if component == "agent":
        return (
            ("bin/romi-agent", "@bin/romi-agent", 0o755),
            ...
            *common,
            ...
            ("deploy/agent/romi-agent.openrc.in", "deploy/agent/romi-agent.openrc.in", 0o644),
        )
    ...
    for member, source, mode in component_sources(component):
        if source.startswith("@bin/"):
            ...
            try:
                ...
                if component == "agent" and member == README:
                    data = data.replace(b"](../", b"](")
                payload[member] = (data, mode)
            except OSError as error:
                raise ReleaseError(f"cannot read release payload file {path}: {error}") from error
    metadata = component_metadata(identity, component, engine)
    if component == "hub":
        for target in TARGETS:
            if target == TARGET:
                continue
            agent = binary_dir / "agents" / target / "romi-agent"
            if agent.is_file():
                payload[f"agents/{target}/romi-agent"] = (agent.read_bytes(), 0o755)
                metadata["agent_targets"].append(target)
        metadata["agent_targets"].sort()
    payload["release.json"] = (encoded(metadata), 0o644)
...
def build_release(binary_dir: Path, output_dir: Path, identity: Identity, root: Path = ROOT) -> dict:
    ...
    provenance = binary_dir / "build.json"
    native_build = None
    if provenance.is_file():
        build = json.loads(provenance.read_text(encoding="utf-8"))
        if (build.get("source_commit"), build.get("version"), build.get("target"), build.get("smoke")) != (identity.commit, identity.version, TARGET, "success"):
            raise ReleaseError("native build provenance does not match the requested release")
        for name in ("romi-hub", "romi-agent"):
            if build.get("binaries", {}).get(name) != sha256((binary_dir / name).read_bytes()):
                raise ReleaseError(f"{name}: native build digest mismatch")
        if build.get("image") != IMAGES[TARGET] or build.get("native_machine") != TARGET.split('-')[0]:
            raise ReleaseError("native build image or machine mismatch")
        rustc = {"rustc": build["rustc"], "rustc_host": build["rustc_host"]}
        native_build = {key: build.get(key) for key in ("image", "os_release", "cxx", "native_machine")}
    else:
    ...
    manifest = {
        ...
        "integrity": {"algorithm": "sha256", "file": checksum_filename()},
        "provenance": (
            None
...
def write_sha256sums(directory: Path, names) -> None:
    ...
    (directory / checksum_filename()).write_text("".join(lines), encoding="utf-8")
...
def verify_archive(path: Path, identity: Identity, component: str, engine: str) -> dict:
    ...
    optional = {f"agents/{t}/romi-agent" for t in TARGETS if t != TARGET} if component == "hub" else set()
    alternatives = found & optional
    ...
    if component == "hub":
        targets = sorted([TARGET, *[name.split('/')[1] for name in alternatives]])
        if meta.get("agent_targets") != targets:
            raise ReleaseError("Hub Agent target inventory does not match its archive")
        for name in alternatives:
            if not members[name]["data"] or not members[name]["mode"] & 0o111:
                raise ReleaseError(f"{name}: missing executable Agent")
...
def read_sha256sums(path: Path) -> dict:
    ...
    for number, line in enumerate(lines, 1):
        ...
        if name == path.name or name in sums:
...
def verify_release_dir(
    ...
    if manifest.get("integrity") != {"algorithm": "sha256", "file": checksum_filename()}:
    ...
    sums = read_sha256sums(directory / checksum_filename())
    if set(sums) != expected_files - {checksum_filename()}:
        missing = sorted((expected_files - {checksum_filename()}) - set(sums))
        extra = sorted(set(sums) - (expected_files - {checksum_filename()}))
...
def fixture_release_inputs(root: Path) -> Path:
    ...
    for relative in (
        ...
        "deploy/hub/romi-hub.openrc.in",
        "deploy/agent/install.sh",
        "deploy/agent/romi-agent.service.in",
        "deploy/agent/romi-agent.openrc.in",
    ):
        path = root / relative
...
def run_self_checks() -> int:
    ...
    def same_inputs_serialize_identically():
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name in (artifact_filename("hub", "0.1.0"), manifest_filename(CANDIDATE_KIND, "0.1.0"), checksum_filename()):
                assert (first / name).read_bytes() == (second / name).read_bytes(), name

    # The package fixtures are deliberately not executable binaries. Their
    # compiler identity is fixture data, so these pure tooling checks also run
    # on Windows (which owns local Git operations).
    from unittest import mock
    with mock.patch(__name__ + ".rustc_identity", return_value={"rustc": "rustc fixture", "rustc_host": TARGET}):
...
def cmd_package(args) -> int:
    ...
    print(output_dir / checksum_filename())
    return 0
